...
def addmovie(request):
    ...
    if request.method == 'POST':
        ...
        uploaded_logo = request.FILES.get('movie_logo')
        if uploaded_logo:
            print(f"Uploaded image filename: {uploaded_logo.name}")

        return redirect('index')
    context = {'form': form}
    return render(request, 'recommend/movie_form.html', context)
...
def recommend(request):
    ...
    # If new user hasn't rated any movie
    ...
    for movie, rating in user_filtered:
        similar_movies = pd.concat([similar_movies, get_similar(movie, rating, corrMatrix)], ignore_index=True)  # Use concat
